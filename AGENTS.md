...
|---------------|----------|
| < 1 GB        | WAIT — do not compile, sleep and re-check in a loop |
| >= 1 GB and < 2 GB | `-j1` |
| >= 2 GB and < 3 GB | `-j2` |
| >= 3 GB       | `-j3` (never higher, even if more cores exist) |
...
```bash
# Wait until at least 1 GB is available, then pick -j1/-j2/-j3.
while true; do
  AVAIL_MB=$(awk '/MemAvailable/ {print int($2/1024)}' /proc/meminfo)
  echo "Available RAM: ${AVAIL_MB} MB"
  if [ "$AVAIL_MB" -ge 3072 ]; then JOBS=3; break;
  elif [ "$AVAIL_MB" -ge 2048 ]; then JOBS=2; break;
  elif [ "$AVAIL_MB" -ge 1024 ]; then JOBS=1; break;
  else echo "Less than 1GB available, sleeping 60s..."; sleep 60;
  fi
done
...
- If available RAM drops below 1 GB mid-work, stop starting new compiles; wait (`sleep 60` loop) until it recovers.
- Never exceed `-j3`.
