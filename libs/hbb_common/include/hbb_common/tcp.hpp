...
public:
    ...
    // Send a protobuf message (serializes via Message::write_to_bytes).
    // Template for any type with `write_to_bytes() -> std::vector<uint8_t>`.
    template <typename Msg>
    bool send(const Msg& msg) {
        return send_raw(msg.write_to_bytes());
    }
